...
from datafeeder import DataFeeder2
from hparams import hp, hparams_debug_string
from model import M2
...
def train(log_dir, args):
    ...
    with tf.variable_scope('datafeeder') as scope:
        feeder = DataFeeder2(coord, hp.train2_data_path)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if step % args.checkpoint_interval == 0:
                    ...
                    log('Saving audio...')
                    spectrogram = sess.run(model.pred_spec[0])
                    waveform = audio.inv_spectrogram(spectrogram.T)
                    audio.save_wav(waveform, os.path.join(log_dir, 'step-%d-output-audio.wav' % step))
                    log('Saved!!!')
